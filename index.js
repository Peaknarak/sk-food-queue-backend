...
io.on('connection', (socket) => {
  ...
  // ✅ รองรับการส่งข้อความผ่าน Socket (สำหรับ UI เก่า) — บันทึก DB + broadcast
...
// ส่งข้อความ (HTTP) — บันทึก + broadcast ไปยัง room ของออเดอร์
  ...
  if (type === 'vendor' && vendorId) {
    ...
    if (!user) {
      await prisma.vendor.upsert({
        ...
        update: {},
        create: { id: String(vendorId), name: `Vendor ${vendorId}` },
      });
      user = await prisma.user.create({
        data: { id: String(vendorId), type: 'vendor', name: `Vendor ${vendorId}`, vendorId: String(vendorId) },
      });
    }
    return res.json({ ok: true, user });
  }
...
app.post('/orders', async (req, res) => {
  ...
  try {
    ...
    for (const it of items) {
      const m = await prisma.menuItem.findFirst({ where: { id: it.menuItemId, vendorId } });
